...
//   // Students get their per-exam attempt quota so the UI can show "2 of 5
//   // attempts used" and disable Start before they even try (the backend
//   // still enforces this authoritatively at attempt-start time).
//   let attemptInfo = null;
//   if (req.user.role === "student") {
//     const { maxAttemptsPerExam } = await getAttemptLimitContext(req.user._id);
...
//     attemptInfo = {
//       used,
//       max: maxAttemptsPerExam || null, // null = unlimited
//       remaining: maxAttemptsPerExam
//         ? Math.max(0, maxAttemptsPerExam - used)
//         : null
//     };
//   }
...
//       new ApiResponse(
//         200,
//         { exam, sections, questionCount, attemptInfo },
//         "Exam fetched"
//       )
//     );
// });
...
export const createExam = asyncHandler(async (req, res) => {
  ...
  const exam = await Exam.create({
    ...
    instructions: Array.isArray(instructions) ? instructions : [],
    isPremium: Boolean(req.body.isPremium),
    isSectionTimed: Boolean(req.body.isSectionTimed),
    createdBy: req.user._id
  });
  ...
  if (!exam.isPublished) {
    ...
    if (exam.isSectionTimed) {
      ...
      if (questionsWithoutSection.length > 0) {
        throw new ApiError(
          400,
          "This exam uses section-wise timing, so every question must be assigned to a section before publishing."
        );
      }

      const sections = await Section.find({ examId: exam._id }).lean();
      const sectionIdsWithQuestions = new Set(
        questions.map((q) => String(q.sectionId))
      );
      const missingDuration = sections.filter(
        (s) => sectionIdsWithQuestions.has(String(s._id)) && !s.durationMinutes
      );
      if (missingDuration.length > 0) {
        throw new ApiError(
          400,
          `Set a duration for every section with questions before publishing (missing: ${missingDuration
            .map((s) => s.title)
            .join(", ")}).`
        );
      }
      ...
      if (totalSectionMinutes > exam.durationMinutes) {
        throw new ApiError(
          400,
          `The total duration of all sections (${totalSectionMinutes} minutes) cannot exceed the exam duration (${exam.durationMinutes} minutes).`
        );
      }
    }
  }
...
export const createSection = asyncHandler(async (req, res) => {
  ...
  const { title, order, durationMinutes } = req.body;
  const existingCount = await Section.countDocuments({ examId: exam._id });
  ...
  if (exam.isSectionTimed && sectionDuration !== null) {
    const currentSections = await Section.find({ examId: exam._id }).lean();
    const totalDuration = currentSections.reduce(
      (sum, section) => sum + (section.durationMinutes || 0),
      0
    );
    if (totalDuration + sectionDuration > exam.durationMinutes) {
      throw new ApiError(
        400,
        `The total section time (${totalDuration + sectionDuration} minutes) exceeds the exam duration (${exam.durationMinutes} minutes).`
      );
    }
  }
...
export const updateSection = asyncHandler(async (req, res) => {
  ...
  if (req.body.durationMinutes !== undefined) {
    const nextDuration =
      req.body.durationMinutes !== null && req.body.durationMinutes !== ""
        ? Number(req.body.durationMinutes)
        : null;

    const exam = await Exam.findById(req.params.examId).lean();
    if (exam?.isSectionTimed && nextDuration !== null) {
      const totalDuration = await Section.aggregate([
        { $match: { examId: exam._id, _id: { $ne: section._id } } },
        { $group: { _id: null, total: { $sum: "$durationMinutes" } } }
      ]);
      const existingTotal = totalDuration[0]?.total || 0;
      if (existingTotal + nextDuration > exam.durationMinutes) {
        throw new ApiError(
          400,
          `Section duration exceeds the exam duration (${exam.durationMinutes} minutes).`
        );
      }
    }

    section.durationMinutes = nextDuration;
  }
