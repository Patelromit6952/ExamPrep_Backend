...
/**
 * Verifies the caller's JWT and attaches the corresponding user to
 * req.user. Accepts the token from EITHER source, checked in this order:
 *   1. `Authorization: Bearer <token>` header - used by the Electron build,
 *      where a packaged app's cookie persistence across a file:// origin
 *      talking to a remote HTTPS API can be unreliable.
 *   2. The httpOnly `token` cookie - used by the web build.
 * This lets the exact same backend serve both without special-casing.
 */
export const protect = asyncHandler(async (req, res, next) => {
